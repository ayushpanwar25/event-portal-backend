...
router.post("/register", async (req, res) => {
	const errors = [];
	const validateError = validateClub({ name: req.body.name, email: req.body.email, password: req.body.password });
	if (validateError) {
...
router.post("/signin", async (req, res) => {
	...
	if(!passValid) {
		...
		return res.send({ success: true, user: { name: club.name, email: club.email } });
	}
});
...
router.post("/resetpassword", verifyClub, async (req, res) => {
	...
	Club.findByIdAndUpdate(req.session.userId, { password: await argon2.hash(req.body.password) })
	.then((club => {
		if(!club) {
