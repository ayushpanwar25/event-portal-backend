...
router.post("/signin", async (req, res) => {
	...
	if(!passValid) {
		...
		return res.send({ success: true, user: { regNo: User.regNo, name: User.name, email: User.email } });
	}
});
...
router.post("/resetpassword", verifyUser, async (req, res) => {
	...
	User.findByIdAndUpdate(req.session.userId, { password: await argon2.hash(req.body.password) })
	.then((User => {
		if(!User) {
